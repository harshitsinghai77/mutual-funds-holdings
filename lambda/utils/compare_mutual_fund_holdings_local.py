...
class MutualFundHoldingsLocal:

    def __init__(self) -> None:
        self.database = 'mutual_funds.db'
        with LocalSQLiteDB(self.database) as db:
            db.create_table()
    ...
    def get_holdings_from_db(self, mutual_fund_name):
        with LocalSQLiteDB(self.database) as db:
            retrieve_data = db.retrieve_holdings(mutual_fund_name)
            if retrieve_data:
    ...
    def get_holdings(self, mutual_fund_search_id):
        ...
        if not mutual_fund_holdings:
            print("mutual fund not found in DB")
            ...
            if not mutual_fund_holdings:      
                raise ValueError("No mutual fund holding found from external API")  
            ...
            with LocalSQLiteDB(self.database) as db:
                db.insert_data_into_holdings(search_id=mutual_fund_search_id, fund_name=fund_name, logo_url=logo_url, holdings=mutual_fund_holdings)

        return mutual_fund_info, mutual_fund_holdings
    
    def get_mutual_fund_info(self, mutual_fund_search_id):
        with LocalSQLiteDB(self.database) as db:
            mutual_fund_info = db.retrieve_mutual_funds_by_id(mutual_fund_search_id=mutual_fund_search_id)
